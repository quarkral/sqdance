...
public class ToolBox {
	...
	// return true if it's a inner column vertical swap movement
	public static boolean isVerticalMovement(Point start, Point end) {
		Point diff = ToolBox.pointsDifferencer(start, end);

		return (Math.abs(diff.x) < 0.001) && ((Math.abs(diff.y) - LineStrategy.VERTICAL_GAP) < 0.001);
	}
}
