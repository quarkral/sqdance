...
public class OldConveyorPlayer implements sqdance.sim.Player {
    ...
    public int dancers_per_conveyor = 4;
    private final int CONVEYOR_FREQUENCY = 3;
    private final boolean LSD_OPTIMIZATION = false;
    ...
    public Point[] play(Point[] dancers, int[] scores, int[] partner_ids, int[] enjoyment_gained) {
        ...
        if (mode == 0) {
            ...
            if (play_counter <= 10 || (LSD_OPTIMIZATION && is_lowest_scoring_dancer_scoring_bigly(scores, enjoyment_gained))) {
                play_counter += 1;
                return instructions;
            } else {
                play_counter = 0;
