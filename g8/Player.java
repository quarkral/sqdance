...
public class Player implements sqdance.sim.Player {
    ...
    private double row2 = 0;
    private double row3 = 0;
    private double row4 = 0;    
    
    private boolean waveDown = true;
    ...
    public void init(int d, int room_side) {
       ...
       this.total_enjoyment = new int[d];
    ...
    public Point[] play(Point[] dancers, int[] scores, int[] partner_ids, int[] enjoyment_gained) {
        ...
        if (d == 2){
            ...
            int lowest_enjoyment_current = enjoyment_gained[getLowestEnjoymentIndex()];
            if(lowest_enjoyment_current == 6 || lowest_enjoyment_current == 4){
                swap = false;
            }
                for (int i = 0; i < d; i++) {
                    total_enjoyment[i] += enjoyment_gained[i];
                    Point curr = dancers[i];
                    Point nextPos;
                    nextPos = (swap ? swapGridPartners(i, dancers) : curr);
                    if(!wave || swap){    
                        instructions[i] = new Point(nextPos.x - curr.x, nextPos.y - curr.y);
                        instructions[i] = makeValidMove(instructions[i]);
                        continue;
                    ...
                    if(Math.abs(curr.y - row1) < 0.0001){

                        instructions[i] = new Point(0, PARTNER_DIST - MIN_DIST - 0.0000001);
                        if(!waveDown){
                            instructions[i] = new Point(0, 0 - (PARTNER_DIST - MIN_DIST));
                        }

                        instructions[i] = makeValidMove(instructions[i]);
                        
                        continue;
                    }else if(Math.abs(curr.y - row2) < 0.0001){
                        instructions[i] = new Point(0, PAIR_DIST + PARTNER_DIST - MIN_DIST - MIN_DIST);
                        if(!waveDown){
                            instructions[i] = new Point(0, 0 - (PAIR_DIST + PARTNER_DIST - MIN_DIST - MIN_DIST));
                        }
                        instructions[i] = makeValidMove(instructions[i]);
         
                        continue;
                    }else if(Math.abs(curr.y - row3) < 0.0001){
                        instructions[i] = new Point(0,  PAIR_DIST + PARTNER_DIST - MIN_DIST - MIN_DIST);
                        if(!waveDown){
                            instructions[i] = new Point(0, 0 - (PAIR_DIST + PARTNER_DIST - MIN_DIST - MIN_DIST));
                        }
                        instructions[i] = makeValidMove(instructions[i]);
                        continue;
                    }else if(Math.abs(curr.y - row4) < 0.0001){
                        instructions[i] = new Point(0, PARTNER_DIST - MIN_DIST);
                        if(!waveDown){
                            instructions[i] = new Point(0, 0 - (PARTNER_DIST - MIN_DIST));
                ...
                if(waveDown){
                    row1 = row3 + PAIR_DIST + PARTNER_DIST - MIN_DIST - MIN_DIST;
                    row2 = row4 + PARTNER_DIST - MIN_DIST;
                    row3 = row2 + MIN_DIST;
                    row4 = row3 + PARTNER_DIST;
                }else{
                    row1 = row3 - (PAIR_DIST + PARTNER_DIST - MIN_DIST - MIN_DIST);
                    row2 = row4 - (PARTNER_DIST - MIN_DIST);
                    row3 = row2 - MIN_DIST;
                ...
                if((row4+PARTNER_DIST) >= room_side){
                    double temp = row1;
                    row1 = row2;
                    row2 = temp;
                    row3 = row2 - MIN_DIST;
    ...
    private int getLowestEnjoymentIndex(){
        int min = 9000;
        int minIndex = 0;
        for(int i=0;i<d;i++){
            if(total_enjoyment[i] < min){
                min = total_enjoyment[i];
                minIndex = i;
            }
        }
        return minIndex;
     ...
     *    i -- current dancer id
     */
    private Point swapGridPartners(int i, Point[] dancers) {
        Point nextPos;
        
        // otherwise go to the next position
        int new_idx = (i + 1) % d;
        nextPos = dancers[new_idx];
        ...
        for (int i = 1; i < dancers; i++) {
            if (i == active_dancers) {
                ...
                if (i == active_dancers + (dancers - active_dancers) / 2) {
                    ...
                    double x;
                    if(wave){
                        x = curr.x + PARTNER_DIST;
                    }else{
                        x = curr.x + MIN_DIST;
                    }

                    if ( x > room_side - gridMargins) {
                        if (!ydir) {
